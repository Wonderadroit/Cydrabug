from cydra.program_intake import (
    AcquiredResource, AuthorityClass, ResourceKind, ScopeStatus,
    build_program_contract, canonical_resource_id, contract_to_system_model,
)
...
def test_migrated_program_contract_is_fail_closed():
    ...
    program=build_program_contract(
        program_id="audit-competition-ens", display_name="ENS",
        primary_locator=locator, resources=(
            __import__("cydra.program_intake",fromlist=["resource_from_acquisition"]).resource_from_acquisition(
                kind=ResourceKind.PROGRAM, acquired=resource, adapter="fixture", authority=AuthorityClass.AUTHORITATIVE
            ),
        ),
    )
...
def test_explicit_out_of_scope_context_remains_context_only():
    from cydra.program_intake import resource_from_acquisition
    primary_locator="https://immunefi.com/audit-competition/audit-competition-ens/scope/"
    primary=resource_from_acquisition(kind=ResourceKind.PROGRAM,acquired=AcquiredResource(primary_locator,"x","fixture"),adapter="fixture",authority=AuthorityClass.AUTHORITATIVE)
    dep=resource_from_acquisition(kind=ResourceKind.REPOSITORY,acquired=AcquiredResource("https://github.com/example/dep","x","fixture"),adapter="fixture",authority=AuthorityClass.PROJECT,parent_resource_id=primary.resource_id,scope=ScopeStatus.OUT_OF_SCOPE,required=False)
    contract=build_program_contract(program_id="ens",display_name="ENS",primary_locator=primary_locator,resources=(primary,dep))
