...
def _fake_tools(monkeypatch, head, tree, clean=True, hashes=None):
    ...
    monkeypatch.setattr("cydra.target_environment.observe_version", lambda root, name: {
        "node": "v22.23.2",
...
def test_runner_blocks_before_materialization_when_bootstrap_tool_missing(monkeypatch, tmp_path):
    ...
    def unexpected_run(*args, **kwargs):
        ...
        raise AssertionError("materialization executed despite failed bootstrap")

    monkeypatch.setattr("cydra.ens_build_runner.subprocess.run", unexpected_run)
    monkeypatch.setattr("cydra.target_environment.subprocess.run", unexpected_run)

    run = run_ens_build(target)

    assert not run.verified
    assert run.commands[0].command == ("pnpm", "install", "--frozen-lockfile")
    assert run.commands[0].returncode == 127
    assert run.preparation.materialization_status == "BOOTSTRAP_BLOCKED"
...
def test_runner_materializes_before_verifying_target_local_tool(monkeypatch, tmp_path):
    ...
    def runner_run(argv, **kwargs):
        ...
        if command == ["node", "--version"]:
            ...
            Result.stdout = "cda79acaad59711b943fc68207ebb3f1d0ff8596" if command[-1] != "HEAD^{tree}" else "8e0d79dac1ab4b4fdb80d6afed810087ae9f00ba"
        elif command[:3] == ["git", "status", "--porcelain"]:
            Result.stdout = ""
        elif command[-1].startswith("HEAD:"):
            Result.stdout = HASHES[command[-1][5:]]
        else:
            Result.stdout = ""
        return Result()

    monkeypatch.setattr("cydra.ens_build_runner.subprocess.run", runner_run)

    def observe(root, name):
        if name == "tsgo" and not installed["value"]:
            return None
        return {"node": "v22.23.2", "pnpm": ENS_PNPM_VERSION, "tsgo": ENS_TSGO_VERSION}[name]

    monkeypatch.setattr("cydra.target_environment.observe_version", observe)
    monkeypatch.setattr("cydra.target_environment.subprocess.run", runner_run)

    run = run_ens_build(target)

    assert run.preparation.materialization_returncode == 0
    assert run.environment.ready
    assert run.commands[0].command == ("pnpm", "install", "--frozen-lockfile")
    assert run.commands[0].returncode == 0
    assert run.commands[1].command == CANONICAL_COMMANDS[1]
...
def test_runner_stops_at_first_failed_canonical_command(monkeypatch, tmp_path):
    ...
    def fake_run(root, command, timeout):
        ...
        return CommandObservation(tuple(command), 0, "SUCCEEDED")

    monkeypatch.setattr("cydra.ens_build_runner._run", fake_run)
    run = run_ens_build(target)

    assert calls == list(CANONICAL_COMMANDS[1:2])
    assert not run.verified
