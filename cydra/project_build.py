...
Compiler/build identity is protocol-specific and therefore remains explicit evidence.
"""
...
def _node_toolchain(root: Path) -> ToolchainSpec:
    for name in (".nvmrc", ".node-version"):
        v = _read(root, name)
        if v and v.strip():
            return ToolchainSpec("node", v.strip().splitlines()[0], name, "node")
    try:
        p = json.loads(_read(root, "package.json") or "{}")
...
def _foundry_toolchain(root: Path) -> ToolchainSpec:
    ...
    # Foundry permits a pinned solc in profile sections or a solc_version setting.
    # Keep the raw declared value only when it can be identified unambiguously.
    ...
    return ToolchainSpec("solc", None, "foundry.toml:compiler-unspecified", "forge")
...
def detect_project(root: str | Path) -> BuildProfile:
    ...
    if (root / "foundry.toml").is_file():
        return BuildProfile(
            ...
            "solc-json-ast", "solidity", _foundry_toolchain(root),
            ("foundry.toml", "foundry.lock"),
        )
    if (root / "Cargo.toml").is_file():
        return BuildProfile(
            "cargo", ("cargo", "check"), ("target",), "none", "rust",
            ToolchainSpec("rustc", None, "repository-default", "cargo"),
...
def _version(executable: str | None, root: Path) -> str | None:
    ...
    if not path:
        return None
    try:
        r = subprocess.run([path, "--version"], cwd=root, capture_output=True, text=True, timeout=20, check=False)
    except (OSError, subprocess.TimeoutExpired):
        return None
    text = (r.stdout or r.stderr).strip()
    return text.splitlines()[0][:1000] if r.returncode == 0 and text else None


def _dependency_lock_files(profile: BuildProfile) -> tuple[str, ...]:
    return tuple(x for x in profile.config_files if "lock" in x.lower())


def build_identity(root: str | Path, revision: str, repository: str = "") -> BuildIdentity:
    root = Path(root).resolve()
    profile = detect_project(root)
    observed = _version(profile.toolchain.executable, root) if profile.toolchain else None
    return BuildIdentity(
        repository,
        revision,
        profile,
        _fingerprint_files(root, profile.config_files),
        profile.toolchain,
        {profile.toolchain.tool: observed} if profile.toolchain else {},
        _dependency_lock_files(profile),
    )


def _discover_artifact_files(root: Path, profile: BuildProfile) -> tuple[str, ...]:
    found: list[str] = []
    for relative_root in profile.artifact_roots:
...
class ProjectBuilder:
    def __init__(self, root: str | Path, profile: BuildProfile | None = None):
        ...
        self.profile = profile or detect_project(root)
    ...
    def build(self, *, command: Sequence[str] | None = None, timeout: int = 600) -> BuildResult:
        ...
        observed = _version(argv[0], self.root)
        config = _fingerprint_files(self.root, self.profile.config_files)
        if executable is None:
            return BuildResult(
                "TOOLCHAIN_UNAVAILABLE", argv, 127, "", f"executable unavailable: {argv[0]}",
                self.profile, {}, (), observed, config, "NOT_ESTABLISHED",
            )
        try:
            r = subprocess.run(list(argv), cwd=self.root, capture_output=True, text=True, timeout=timeout, check=False)
        except subprocess.TimeoutExpired as e:
            return BuildResult("TIMEOUT", argv, 124, e.stdout or "", e.stderr or "", self.profile, {}, (), observed, config, "NOT_ESTABLISHED")

        status = "SUCCEEDED" if r.returncode == 0 else "FAILED"
        artifacts = _parse_foundry_build_info(self.root) if self.profile.system == "foundry" and status == "SUCCEEDED" else {}
        artifact_paths = _discover_artifact_files(self.root, self.profile) if status == "SUCCEEDED" else ()
        lock_files = _dependency_lock_files(self.profile)
        reproducibility = "LOCKED_DEPENDENCIES" if lock_files and status == "SUCCEEDED" else ("BUILD_SUCCEEDED" if status == "SUCCEEDED" else "PARTIAL")
        dependency_fingerprint = _fingerprint_files(self.root, lock_files)
        dependency_metadata = {"lock_files": lock_files} if lock_files else None
        return BuildResult(
            status, argv, r.returncode, r.stdout, r.stderr, self.profile, artifacts,
            artifact_paths, observed, config, reproducibility, dependency_metadata,
