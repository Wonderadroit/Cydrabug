...
def validate_result_binding(result: ExternalExecutionResult, request: ExecutionRequest) -> None:
    ...
    if payload.get("execution_id") != result.execution_id or payload.get("request_digest") != result.request_digest or payload.get("outcome") != result.outcome:
...
class ExternalExecutionGateway:
    def __init__(self, persist_request: Callable[[ExecutionRequest], object] | None = None, set_execution_state: Callable[[ExecutionRequest, str], object] | None = None, get_execution_state: Callable[[ExecutionRequest], str | None] | None = None, persist_result: Callable[[ExecutionRequest, ExternalExecutionResult], object] | None = None):
        ...
        self._executed_digests = set()
        self._local_states = {}
        self._execution_capabilities = {}
    ...
    def _state(self, request, state):
        ...
        if state not in _ALLOWED_TRANSITIONS.get(current, frozenset()): raise RuntimeError(f"invalid external execution lifecycle transition: {current or 'ABSENT'} -> {state}")
    ...
    def _mark_outcome_unrecorded(self, request):
        try: self._state(request, "OUTCOME_UNRECORDED")
        except Exception: self._local_states[request.digest] = "OUTCOME_UNRECORDED"

    def execute(self, name: str, request: ExecutionRequest, *, authorization, investigation_authorization=None):
        ...
        try:
            ...
            raise RuntimeError("external execution succeeded but durable result/completion recording failed") from exc
        self._executed_digests.add(request.digest)
        return result

    def rehydrate_result(self, name: str, request: ExecutionRequest, payload: Mapping[str, object]):
        current = self._current_state(request)
        ...
        result = self.adapter(name).rehydrate_result(payload=payload, request=request)
        validate_result_binding(result, request)
